...
class AIService:
    ...
    async def analyze_image(self, image_url: str,user:Dict) -> Dict:
                ...
                messages=[
                    {
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": """Analyze this image and extract ONLY food items with their amounts. Follow these rules:
1. List ONLY food items that are clearly visible and identifiable
2. For each item, provide:
   - Name: Use singular form (e.g., 'apple' not 'apples')
   - Amount: If visible, include the quantity (e.g., '2 pieces', '500g', '1 kg')
   - If amount is not visible, use 'some' as the amount
3. Format each item as: "item_name (amount)"
4. If no food items are visible, respond with "No food items detected"
5. Do not include any other descriptions or analysis

Response MUST be a valid JSON object with this exact structure:
{
    "items": [  # Include for update_inventory
            {
                "name": "string",
...
"""
            ...
            new_items = "Add in my inventory "+response.choices[0].message.content.strip()
            classification = await self.classify_message(new_items,user["preferences"])
            
            response=await self.process_classified_message(classification,user,new_items)
            return response
        except Exception as e:
    ...
    async def classify_message(self, message: str, user_preferences: Dict) -> Dict:
        ...
        3. update_inventory - User wants to add or update kitchen inventory items, for example, I have 2 kg of tomatoes, I have 5 tomatoes
        4. get_inventory - User wants to view their current inventory
        5. remove_inventory - User wants to remove items from their inventory , for example, I used 2 kg of tomatoes, I ate 5 tomatoes
