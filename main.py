...
# Dictionary to store message processing locks for each user
user_message_locks = {}
...
async def whatsapp_webhook(
    ...
    # Check if user has an ongoing message processing
    if phone_number in user_message_locks and user_message_locks[phone_number].locked():
        # Send busy message to user
        await whatsapp_service.send_text_message(
            to_number=phone_number,
            message="⏳ I'm still processing your previous message. Please wait for my response before sending another message."
        )
        return {"status": "busy", "message": "User has an ongoing message processing"}
    
    # Create a lock for this user if it doesn't exist
    if phone_number not in user_message_locks:
        user_message_locks[phone_number] = asyncio.Lock()
    
    # Acquire the lock for this user
    async with user_message_locks[phone_number]:
        try:
            # Get or create user
            ...
            if parsed_message["type"] == "text":
                ...
                image_analysis = await ai_service.analyze_image(parsed_message["media_url"], user)
                await whatsapp_service.send_text_message(
                    to_number=phone_number,
                    message=image_analysis["response"]
                )
            ...
            # The lock will be automatically released when the async with block ends
            pass
